...
$doorName = DoorContext::doorName();

Layout::header('Türkontrolle – Scanner');
?>
  ...
  <div class="container py-5" style="max-width: 800px;">
    ...
    <div class="text-center mx-auto mb-4" style="max-width: 760px;">
      ...
      <p class="text-muted" style="font-size: 1.05rem; line-height: 1.7;">
        Willkommen, <strong><?= e($doorName) ?></strong> 
        <span class="mx-2">·</span>
        <a href="/door_logout.php" class="text-danger" style="text-decoration: none; font-weight: 600;">Abmelden</a>
      </p>
    </div>
      ...
      <div class="card-body p-4">
        <div class="alert alert-warning mb-4">
          ...
          <ul class="mb-0 mt-2">
            ...
            <li>QR-Code von Ticket in Kamera halten</li>
            <li>Ticket wird automatisch entwertet</li>
            <li>Ergebnis wird angezeigt</li>
          </ul>
        </div>
  ...
  <script>
        ...
        async function startCamera() {
            ...
            try {
                ...
                // Hole verfügbare Kameras mit Browser API
                const devices = await navigator.mediaDevices.enumerateDevices();
                const videoInputDevices = devices.filter(device => device.kind === 'videoinput');
                console.log('Verfügbare Kameras:', videoInputDevices);
                ...
                for (const device of videoInputDevices) {
                    ...
                    if (label.includes('back') || 
                        ...
                        console.log('Rückkamera ausgewählt:', device.label);
                        break;
                    }
                }
                
                toggleBtn.textContent = '⏹️ Kamera stoppen';
                cameraActive = true;
                ...
                await codeReader.decodeFromVideoDevice(selectedDeviceId, 'video', (result, err) => {
                    if (result && !isScanning) {
                        ...
                        console.log('QR-Code erkannt:', text);
                        
                        // Versuche URL-Format zu parsen: /ticket/verify.php?pid=...&sig=...
                        try {
                            ...
                            if (pid && sig) {
                                ...
                                handleTicketScan(pid, sig);
                                return;
                            }
                        } catch (e) {
                            // Nicht als URL parsebar, versuche altes Format
                        }
                        
                        // Fallback: Altes Format PID|SIGNATURE
        ...
        async function handleTicketScan(pid, sig) {
            ...
            try {
                const response = await fetch('/door_validate_ticket.php', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    icon.textContent = '✓';
                    status.textContent = 'Ticket entwertet';
                    message.textContent = data.message || 'Ticket erfolgreich entwertet';
                    
                    details.innerHTML = `
                        <div class="result-detail-row">
                            <span class="result-label">Person:</span>
                            <span class="result-value">${escapeHtml(data.person_name || '')}</span>
                        </div>
                        <div class="result-detail-row">
                            <span class="result-label">Ticket-ID:</span>
                            <span class="result-value" style="font-size: 0.8rem;">${escapeHtml(data.person_id || '')}</span>
                        </div>
                        <div class="result-detail-row">
                            <span class="result-label">Zeit:</span>
                            <span class="result-value">${escapeHtml(data.valid_until || '')}</span>
                        </div>
                    `;
                    ...
                    message.textContent = data.message || 'Ticket konnte nicht entwertet werden';
                    ...
                    if (data.valid_until) {
                        detailHtml += `
                            <div class="result-detail-row">
                                <span class="result-label">Bereits entwertet am:</span>
                                <span class="result-value">${escapeHtml(data.valid_until)}</span>
                            </div>
                        `;
                    }
        ...
        function playSuccessSound() {
            // Einfaches Beep mit Web Audio API
            try {
